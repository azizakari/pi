...
async function sendMail(user, callback) {
  ...
  let mailOptions = {
    ...
    subject: "Requesting New product Order "+user.name, // Subject line
    html: `
    <head>
    <style>
      table {
    ...
    <h2>So we (mondher ${user.quantityNumber} amount of units from ${user.name} )</h2><br>
    ...
    <h3>Info* : </h3>
    <h4>If there is any issue reagrding the oder please be free to contact us or email us (fedibrahem14@gmail.com) 😃 </h4>
    </body>
    `
  };
...
async function sendmailOutOfStock(user, callback) {
  ...
  let mailOptions = {
    ...
    to: user.email, // list of receivers
    subject: "Requesting New product Oder "+user.name, // Subject line
    html: `
    <head>
    <style>
      table {
    ...
    <h2>So we (mondher like to request ${user.quantityNumber} amount of units from ${user.name} )</h2><br>
    <h3>Please reply back if the this oder is verified.</h3>
